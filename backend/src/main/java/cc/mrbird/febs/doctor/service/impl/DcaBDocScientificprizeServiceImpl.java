    ...
    public IPage<DcaBDocScientificprize> findDcaBDocScientificprizes(QueryRequest request, DcaBDocScientificprize dcaBDocScientificprize) {
        try {
            ...
            if (StringUtils.isNotBlank(dcaBDocScientificprize.getCreateTimeFrom()) && StringUtils.isNotBlank(dcaBDocScientificprize.getCreateTimeTo())) {
                ...
                if (dcaBDocScientificprize.getIsDeletemark() != null && dcaBDocScientificprize.getIsDeletemark().equals(1)) {
                        queryWrapper.apply(" dca_b_doc_scientificprize.user_account in (\n" +
                                "select user_account  from dca_b_doc_user  where  IFNULL(cz_date,'9999-01-01')  >= dca_b_doc_scientificprize.sr_prize_date and dca_b_doc_scientificprize.sr_prize_date >= IFNULL(in_date,'1900-01-01') )");
                }
                if (dcaBDocScientificprize.getIsDeletemark() != null && dcaBDocScientificprize.getIsDeletemark().equals(2)) {
                        queryWrapper.apply(" dca_b_doc_scientificprize.user_account in (\n" +
                                "select user_account  from dca_b_doc_user  where  IFNULL(cz_date,'9999-01-01')  < dca_b_doc_scientificprize.sr_prize_date or dca_b_doc_scientificprize.sr_prize_date < IFNULL(in_date,'1900-01-01') )");
                }
