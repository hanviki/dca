...
public class DcaBDocSciencepublishServiceImpl extends ServiceImpl<DcaBDocSciencepublishMapper, DcaBDocSciencepublish> implements IDcaBDocSciencepublishService {
    ...
    public IPage<DcaBDocSciencepublish> findDcaBDocSciencepublishs(QueryRequest request, DcaBDocSciencepublish dcaBDocSciencepublish) {
        try {
            ...
            if(dcaBDocSciencepublish.getIsDeletemark()!=null && dcaBDocSciencepublish.getIsDeletemark().equals(1)){
                    queryWrapper.apply(" dca_b_doc_sciencepublish.user_account in (\n" +
                            "select user_account  from dca_b_doc_user  where  IFNULL(cz_date,'9999-01-01')  >= dca_b_doc_sciencepublish.paper_publishdate and dca_b_doc_sciencepublish.paper_publishdate >= IFNULL(in_date,'1900-01-01') )");
            }
                if(dcaBDocSciencepublish.getIsDeletemark()!=null && dcaBDocSciencepublish.getIsDeletemark().equals(2)){
                        queryWrapper.apply(" dca_b_doc_sciencepublish.user_account in (\n" +
                                "select user_account  from dca_b_doc_user  where  IFNULL(cz_date,'9999-01-01')  < dca_b_doc_sciencepublish.paper_publishdate or dca_b_doc_sciencepublish.paper_publishdate < IFNULL(in_date,'1900-01-01') )");
                }
