    ...
    public IPage<DcaBDocPublicarticle> findDcaBDocPublicarticles(QueryRequest request, DcaBDocPublicarticle dcaBDocPublicarticle) {
        try {
            ...
            if (StringUtils.isNotBlank(dcaBDocPublicarticle.getCreateTimeFrom()) && StringUtils.isNotBlank(dcaBDocPublicarticle.getCreateTimeTo())) {
                ...
                if (dcaBDocPublicarticle.getIsDeletemark() != null && dcaBDocPublicarticle.getIsDeletemark().equals(1)) {
                        queryWrapper.apply(" dca_b_doc_publicarticle.user_account in (\n" +
                                "select user_account  from dca_b_doc_user  where  IFNULL(cz_date,'9999-01-01')  >= dca_b_doc_publicarticle.cb_date and dca_b_doc_publicarticle.cb_date >= IFNULL(in_date,'1900-01-01') )");
                }
                if (dcaBDocPublicarticle.getIsDeletemark() != null && dcaBDocPublicarticle.getIsDeletemark().equals(2)) {
                        queryWrapper.apply(" dca_b_doc_publicarticle.user_account in (\n" +
                                "select user_account  from dca_b_doc_user  where  IFNULL(cz_date,'9999-01-01')  < dca_b_doc_publicarticle.cb_date or dca_b_doc_publicarticle.cb_date < IFNULL(in_date,'1900-01-01') )");
                }
