        ...
        for (DcaBUser user : listUser
          ...
          //  DcaBReport dcaBReport = new DcaBReport();
            ...
            List<DcaBQualification> listquli= dcaBQualificationList.stream()
                    .filter(p -> p.getUserAccount().equals(userAccount) &&p.getQualificationName()!=null &&p.getAuditGrade()!=null&& p.getQualificationName().equals("湖北省专业技术职务资格证书")&&p.getAuditGrade()!=null&&p.getAuditGrade().equals("正高级")).collect(Collectors.toList());
            if(listquli.size()<=0){
                listquli= dcaBQualificationList.stream()
                        .filter(p -> p.getUserAccount().equals(userAccount) &&p.getQualificationName()!=null &&p.getAuditGrade()!=null&& p.getQualificationName().equals("湖北省专业技术职务资格证书")&&p.getAuditGrade().equals("副高级")).collect(Collectors.toList());
            }
